...
class BasicDataset(Dataset):
    ...
    def preprocess(cls, pil_img, is_img, size):
        ...
        if len(size) > 0:
            ...
            if (img_trans.shape[0] - x) % 2 == 1 and (img_trans.shape[1] -
                ...
                img_trans = img_trans[jianx:-1 * jianx,
                                      jiany + 1:-1 * jiany, :]
            else:
    ...
    def __getitem__(self, i):
        ...
        if self.probability_dir:
            ...
            return {
                'image':
                torch.from_numpy(image.transpose(
                    (2, 0, 1))).type(torch.FloatTensor),  # 返回tensor类型的图像
                'mask':
                torch.from_numpy(mask.transpose(
                    (2, 0, 1))).type(torch.FloatTensor),  # 返回tensor类型的标签
                'probability_map':
                torch.from_numpy(probability_map.transpose(
                    (2, 0, 1))).type(torch.FloatTensor)  #返回概率图谱
            }
